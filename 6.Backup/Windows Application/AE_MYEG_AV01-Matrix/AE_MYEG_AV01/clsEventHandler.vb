        ...
        Try
            ...
            If pVal.BeforeAction = False Then
                Select Case pVal.MenuUID
                    Case "AE_US"
                        UploadStatement_SBO_MenuEvent(pVal, BubbleEvent)
                    Case "AE_EL"
                        ExpList_SBO_MenuEvent(pVal, BubbleEvent)
                    Case "1292" 'Add Matrix rows
                        If p_oSBOApplication.Forms.ActiveForm.TypeEx = "CSS" Then
                            CustSelection_SBO_MenuEvent(pVal, BubbleEvent)
                        End If
                End Select
            End If
            ...
            Select Case pVal.FormTypeEx
                Case "BUPS"
                    If Not (pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_UNLOAD Or pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_CLOSE) Then
                        ...
                        UploadStatement_SBO_ItemEvent(FormUID, pVal, BubbleEvent, oForm)
                        Exit Sub
                    End If
                Case "EXPL"
                    If Not (pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_UNLOAD Or pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_CLOSE) Then
                        oForm = p_oSBOApplication.Forms.GetForm(pVal.FormType, pVal.FormTypeCount)
                        ExpList_SBO_ItemEvent(FormUID, pVal, BubbleEvent, oForm)
                        Exit Sub
                    End If
                Case "CSS"
                    If Not (pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_UNLOAD Or pVal.EventType = SAPbouiCOM.BoEventTypes.et_FORM_CLOSE) Then
                        oForm = p_oSBOApplication.Forms.GetForm(pVal.FormType, pVal.FormTypeCount)
                        CustSelection_SBO_ItemEvent(FormUID, pVal, BubbleEvent, oForm)
                        Exit Sub
                    End If
            End Select
