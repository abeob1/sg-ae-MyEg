...
Public Class fARBadDeptRelief
    ...
    Private Sub DrawForm()
        Try
            ...
            ' set the form title
            f.Title = "GST AR Bad Debt Relief"
    ...
    Sub AddMatrixToForm(ByVal f As SAPbouiCOM.Form)
        Try
            ...
            oColumn = oColumns.Add("BaseAmount", SAPbouiCOM.BoFormItemTypes.it_EDIT)
            oColumn.TitleObject.Caption = "Base Amount"
            oColumn.Width = 90
            oColumn.Editable = False
    ...
    Private Sub Calculate(oForm As SAPbouiCOM.Form)
        Try
            ...
            Dim dt As DataTable
            If PublicVariable.oCompany.DbServerType = SAPbobsCOM.BoDataServerTypes.dst_HANADB Then
                str = "Call sp_SAPB1Addon_GSTBadDebt ('" + todate + "','" + debitor + "'," + CStr(month) + ",'" + ClaimAmt + "','" + Status + "')"
                dt = Functions.Hana_RunQuery(str)
            Else
                str = "exec sp_SAPB1Addon_GSTBadDebt '" + todate + "','" + debitor + "'," + CStr(month) + ",'" + ClaimAmt + "','" + Status + "'"
            ...
            dt1.Columns.Add("BaseAmount", SAPbouiCOM.BoFieldsType.ft_Float, 10)
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                ...
                dt1.SetValue("BaseAmount", i, dt.Rows(i).Item("BaseAmount").ToString())
            ...
            oMatrix.Columns.Item("BaseAmount").DataBind.Bind("X", "BaseAmount")
...
#Region "Post JE"
    ...
    Public Function CreateBadDebt(oForm As SAPbouiCOM.Form) As String
        Try
            ...
            Dim ck As SAPbouiCOM.CheckBox
            Dim oEdit As SAPbouiCOM.EditText
            Dim Ret As String, BaseSum As Decimal, InvoiceEntry As Integer, RefDate As String, DebAct As String, CreAct As String, Amount As String, TaxCode As String
            ...
            For i As Integer = 1 To oMatrix.RowCount
                ...
                If ck.Checked = True Then
                    ...
                    If oEdit.Value = "NO" Then
                        ...
                        oEdit = oMatrix.Columns.Item("BaseAmount").Cells.Item(i).Specific
                        BaseSum = oEdit.Value

                        If Amount <> 0 Then
                            If OneJE = "N" Then
                                Ret = CreateJE(InvoiceEntry, RefDate, DebAct, CreAct, Amount, TaxCode, BaseSum)
                                If Ret <> "" Then
                                    Return Ret
                                End If
                            Else
                                dtHeader = InsertIntoOJDT(dtHeader, RefDate)
                                dtLine = InsertIntoJDT1(dtLine, DebAct, CreAct, Amount, TaxCode, InvoiceEntry, BaseSum)
                            End If
    ...
    Private Function ReturnInvEntryFromInvNum(DocNum As String) As String
        Try
            Dim dt As DataTable
            If PublicVariable.oCompany.DbServerType = SAPbobsCOM.BoDataServerTypes.dst_HANADB Then
                dt = Functions.DoQueryReturnDT("select ""DocEntry"" from ""OINV"" where ""DocNum""='" + DocNum + "'")
            Else
                dt = Functions.DoQueryReturnDT("select DocEntry from OINV with(nolock) where DocNum='" + DocNum + "'")
            End If
    ...
    Private Function CreateOneJE(dtHeader As DataTable, dtLine As DataTable) As String
        ...
        End Try
    End Function
    Private Function CreateJE(InvoiceEntry As Integer, RefDate As String, DebAct As String, CreAct As String, Amount As String, TaxCode As String, BaseSum As Decimal) As String
        Try
            Dim xmlstr As String = ""
            Dim ds As New DataSet
            Dim ret As String = ""
    ...
    Private Function UpdateNonBadDebtInvoice(JENo As String) As String
        Try
            Dim str As String
            If PublicVariable.oCompany.DbServerType = SAPbobsCOM.BoDataServerTypes.dst_HANADB Then
                str = "update ""OINV"" set ""U_BadDebt""='N' where ""DocNum"" in (select distinct ""U_InvoiceEntry"" from ""JDT1"" where ""TransId""=" + JENo + ")"
            Else
                str = "update OINV set U_BadDebt='N' where Docnum in (select distinct U_InvoiceEntry from JDT1 where TransID=" + JENo + ")"
            End If
