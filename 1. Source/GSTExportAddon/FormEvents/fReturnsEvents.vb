            ...
            If BusinessObjectInfo.BeforeAction = False Then
                Select Case BusinessObjectInfo.Type
                    Case "16" 'Return
                        ...
                        oReturn = PublicVariable.oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oReturns)
                        If BusinessObjectInfo.EventType = SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD Then
                            If oReturn.Browser.GetByKeys(BusinessObjectInfo.ObjectKey) Then
                                For i As Integer = 0 To oReturn.Lines.Count - 1
                                    oReturn.Lines.SetCurrentLine(i)
                                    If oReturn.Lines.BaseType = 15 Then
                                        ...
                                        If oDeliveryNote.GetByKey(oReturn.Lines.BaseEntry) Then
                                            If oDeliveryNote.UserFields.Fields.Item("U_21Day").Value.ToString = "Y" Then
                                                Dim ret As String = ""
                                                ret = CreateJE21Day_Reverse(oDeliveryNote.UserFields.Fields.Item("U_21DayJE").Value.ToString, oReturn.DocDate.ToString("yyyyMMdd"), oDeliveryNote.DocNum.ToString)
                                                If ret <> "" Then
